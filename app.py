...
def optimize_bio_energy(upload_file):
    ...
    for i in range(len(req_bio)):
        if i <= 2:
            model.add_component("cond_"+str(i), pyo.Constraint(expr=req_bio.demand[i] <= sum(kb[j] for j in range(8))))
        else:
            model.add_component("cond_"+str(i), pyo.Constraint(expr=req_bio.demand[i] <= sum(kb[j] for j in range(8, 14))))
